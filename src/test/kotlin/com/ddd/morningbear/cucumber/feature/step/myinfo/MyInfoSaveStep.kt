...
class MyInfoSaveStep {
    ...
    @먼저("내정보를 저장하기 위한 {string}{string}{string}{string}{string}{string} 가 있다")
    fun 내정보를_저장하기_위한_가_있다(docs: String, accountId: String, nickName: String, photoLink: String, memo: String, wakeUpAt: String) {
        this.docs = docs
    ...
    @그러면("호출결과를 확인한다")
    fun 호출결과를_확인한다() {
        result.path("saveMyInfo.accountId").entity(String::class.java).isEqualTo(input["accountId"]!!)
